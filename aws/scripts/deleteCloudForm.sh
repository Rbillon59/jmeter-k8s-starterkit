...
vpcId=`aws ec2 describe-vpcs --filters Name=tag:Name,Values=$stack-VPC --query "Vpcs[].VpcId" --output text --profile $profile`
if [ -z "$vpcId" ]; then
   logit "No vpc found"
else
   logit "Getting the load balancer..."
   objects=(`aws elb describe-load-balancers --query "LoadBalancerDescriptions[?VPCId=='$vpcId'].LoadBalancerName" --output text --profile $profile`)
   if [ ${#objects[@]} -eq 0 ]; then
   ...
   if [ ${#objects[@]} -eq 0 ]; then
      ...
      for i in ${objects[@]}; do
         ...
         for j in ${id[@]}; do
            attachId=(`aws ec2 describe-network-interfaces --network-interface-ids $j --query NetworkInterfaces[].Attachment.AttachmentId --output text --profile $profile`)
            for k in ${attachId[@]}; do
...
objects=(`aws cloudformation list-stacks --query "StackSummaries[?contains(StackName,'$stack')].StackName" --stack-status-filter CREATE_COMPLETE ROLLBACK_COMPLETE UPDATE_COMPLETE UPDATE_ROLLBACK_COMPLETE DELETE_FAILED --output text --profile $profile`)
if [ ${#objects[@]} -eq 0 ]; then
